...
class MultiheadDiff2(nn.Module):
    """
    DiffAttn implementation that can use either standard attention or FlashAttention
    """
    def __init__(
        self,
        embed_dim,
    ...
    def compute_attention(self, q, k, v):
        """Compute attention using either standard or flash attention"""
        if self.use_flash:
            return flash_attn_func(q, k, v, causal=False)
        else:
            # For standard attention, we need to transpose first
            q = q.transpose(1, 2)  # (batch_size, num_heads, tgt_len, head_dim)
            k = k.transpose(1, 2)
            v = v.transpose(1, 2)
            return self.standard_attention(q, k, v).transpose(1, 2)
    ...
    def forward(
        ...
        attn11 = self.compute_attention(q1, k1, v1)  # (batch_size, tgt_len, num_heads, head_dim)
        attn12 = self.compute_attention(q1, k1, v2)  # (batch_size, tgt_len, num_heads, head_dim)
        attn1 = torch.cat([attn11, attn12], dim=-1)  # (batch_size, tgt_len, num_heads, 2 * head_dim)
        
        attn21 = self.compute_attention(q2, k2, v1)  # (batch_size, tgt_len, num_heads, head_dim)
        attn22 = self.compute_attention(q2, k2, v2)  # (batch_size, tgt_len, num_heads, head_dim)
